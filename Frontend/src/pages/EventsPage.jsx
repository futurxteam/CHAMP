...
export default function EventsPage() {
  const { registeredEvents, registerForEvent, getAuthHeaders, isAuthenticated } = useStore();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <p className="text-sm text-surface-500">
            {loading ? "Searching for events..." : `Showing ${filteredEvents.length} events`}
          </p>
        </div>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3].map((n) => (
              <div key={n} className="h-80 rounded-2xl bg-surface-100 animate-pulse border border-surface-200" />
            ))}
          </div>
